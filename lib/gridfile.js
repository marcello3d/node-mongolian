...
MongolianGridFileWriteStream.prototype.flush = function(callback) {
    ...
    if (this._partialIndex) {
        // Temporary fix for GH-24 until new node-mongodb-native release
        var binary = new Binary
        binary.buffer = this._partialChunk.slice(0, this._partialIndex)
        binary.position = binary.buffer.length
