...
MongolianCollection.prototype.save = function(object, callback) {
    if (object._id) {
        this.upsert({_id:object._id}, object, callback)
    } else {
...
MongolianCollection.prototype.mapReduce = function(mapFunction, reduceFunction, options, callback) {
    var command = typeof options == 'string' ? { out: options } : options

    command.mapreduce = this.name
    command.map = mapFunction
    command.reduce = reduceFunction

    this.db.queryCommand(command, safetyNet(callback,function(result) {
        callback(null,new MapReduceResult(this.db, result))
    }))
}

function MapReduceResult(db, result) {
    if (result.results) this.results = result.results
    if (result.db) this.db = result.db
    this.timeMillis = result.timeMillis
    this.counts = result.counts
    if (result.result) this.collection = db.collection(this.result)
}
MapReduceResult.prototype.find = function(callback) {
    if (this.results) {
...
util.inherits(MapReduceResult, MongolianCollection)
