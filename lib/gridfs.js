...
var mongo = require('mongodb'),
    ...
    MongolianGridFile = require('./gridfile')
...
var MongolianGridFS = module.exports = function(db, name) {
    ...
    files.count(safetyNet(null, function(count) {
        if (count < 1000) files.ensureIndex({ filename:1, uploadDate:1 }, { unique:true })
    }))
...
MongolianGridFS.prototype.find = function(searchBy) {
    ...
    } else if (searchBy instanceof mongo.BSONPure.ObjectID) {
        query = { _id:searchBy }
    } else {
        query = searchBy
    }
    var self = this
    return self.files.find(query).mapper(function (document) {
        return new MongolianGridFile(self, document)
    })
}
/**
